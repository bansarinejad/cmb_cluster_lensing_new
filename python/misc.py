...
def get_op_fname_short(op_folder, total_sims, start = -1, end = -1, extrastr = ''):
    if start != -1 and end != -1:
        fname = '%s/%ssims%sto%s%s.npy' %(op_folder, total_sims, start, end, extrastr)
    else:
        fname = '%s/%ssims%s.npy' %(op_folder, total_sims, extrastr)
    return fname
