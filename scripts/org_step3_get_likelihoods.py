...
sys_path_folder='../python/' #Behzad
sys.path.append(sys_path_folder)

import flatsky, org_tools, lensing, foregrounds, misc
...
parser.add_argument('-dataset_fname', dest='dataset_fname', action='store', help='dataset_fname', type=str, default=None)#'../results_tsz_test//nx120_dx1/beam1.2/noise5/10amcutouts/m200mean_2.3e+14//6300clusters/withgaussianfg//TQU//clusters_6300objects_25sims0to25_rsval666.npy')
...
if not add_cluster_tsz:
    ...
    for simcntr in range( totsims ):
        ...
        tsz_estimate = org_tools.stack_rotated_tqu_cutouts(cutouts_rotated_arr_for_tsz_estimation, weights_for_cutouts = None, perform_random_rotation = True)
        ...
        stack_after_tsz_removal = org_tools.stack_rotated_tqu_cutouts(cutouts_rotated_arr, weights_for_cutouts = grad_mag_arr)
...
for keyname in data_stack_dic:
    if (0):
        ...
        show(); sys.exit()
    #if (keyname=='stack'):  #Behzad next two lines are a temp test 28/10/2021
...
for model_fname in model_flist:
    ...
    for simkeyname in model_data:
        ...
        cutouts_rotated_arr = org_tools.get_rotated_tqu_cutouts_simple(cutouts_arr, grad_orien_arr_avg, total_clusters, tqulen)
        stack=org_tools.stack_rotated_tqu_cutouts(cutouts_rotated_arr, weights_for_cutouts = grad_mag_arr)
...
if howmany_jk_samples<min_howmany_jk_samples:
    howmany_jk_samples = min_howmany_jk_samples
#np.random.seed(100)
jk_cov=org_tools.get_jk_covariance(cluster_cutouts_rotated_arr, howmany_jk_samples, weights=cluster_grad_mag_arr, only_T=False)
...
for tqu in range(tqulen):
    if tqu==0:
        jk_cov_temp=jk_cov[0:100,0:100]
    elif tqu==1:
        jk_cov_temp=jk_cov[100:200,100:200]
    elif tqu==2:
        jk_cov_temp=jk_cov[200:300,200:300]
    ...
    for simcntr in sorted(data_stack_dic):
        ...
        for modelcntr, model_keyname in enumerate( sorted( model_dic ) ):
            ...
            loglval = org_tools.get_lnlikelihood(data_vec, model_vec, jk_cov_temp)
            loglarr.append( loglval )
            massarr.append( model_keyname[0] )
        if testing: show(); sys.exit()
        massarr = np.asarray( massarr )
        massarr_mod, larr, recov_mass, snr = org_tools.lnlike_to_like(massarr, loglarr)
    ...
    massarr_mod, combined_larr, combined_recov_mass, combined_snr = org_tools.lnlike_to_like(massarr, combined_loglarr)
    combined_mean_mass, combined_mean_mass_low_err, combined_mean_mass_high_err = org_tools.get_width_from_sampling(massarr_mod, combined_larr)
...
if plot_qu:
    ...
    for simcntr in sorted(data_stack_dic):
        ...
        for modelcntr, model_keyname in enumerate( sorted( model_dic ) ):
            ...
            loglval = org_tools.get_lnlikelihood(data_vec_QU, model_vec_QU, jk_cov_QU) #behzad jk_cov->jk_cov_temp
            loglarr.append( loglval )
            massarr.append( model_keyname[0] )
        if testing: show(); sys.exit()
        massarr = np.asarray( massarr )
