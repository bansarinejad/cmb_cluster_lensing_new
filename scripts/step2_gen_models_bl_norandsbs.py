...
ra=np.linspace(x1+0.5,x2-0.5, nx) #arcmins
dec=np.linspace(x1+0.5,x2-0.5, nx) #arcmins
...
########################
#get beam and noise
bl = tools.get_bl(beamval, el, make_2d = 0, mapparams = mapparams) #Behzad set make_2d=0 to be able to add it to cl further down by doing cl*bl**2
bl_2d = tools.get_bl(beamval, el, make_2d = 1, mapparams = mapparams)
if ilc_file is None:
...
for (cluster_mass, cluster_z) in zip(cluster_mass_arr, cluster_z_arr):
    ...
    for simcntr in range( totiters_for_model ):
        ...
        #for i in range(nclustersorrandoms):
            if not pol:
                ...
                if fg_gaussian:
                    fg_map=np.asarray( [flatsky.make_gaussian_realisation(mapparams, el, cl_fg_dic['T']*bl**2)] )
                else:
                ...
                if fg_gaussian:
                    fg_map_T=flatsky.make_gaussian_realisation(mapparams, el, cl_fg_dic['T']*bl**2)
                    fg_map_Q=flatsky.make_gaussian_realisation(mapparams, el, cl_fg_dic['P']*bl**2)
                    fg_map_U=flatsky.make_gaussian_realisation(mapparams, el, cl_fg_dic['P']*bl**2)
            ...
            #add beam
            cmb_map = np.fft.ifft2( np.fft.fft2(cmb_map) * bl_2d ).real
    ...
    extrastr = '_mass%.3f_z%.3f' %(keyname[0], keyname[1])
    op_fname = misc.get_op_fname_short(op_folder, totiters_for_model, extrastr = extrastr)
